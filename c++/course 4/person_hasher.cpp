...
struct AddressHasher {
    ...
    size_t operator()(const Address &address) const {
        size_t coef = 7340033;
        size_t r1 = shasher(address.street);
        size_t r2 = shasher(address.city);
        size_t r3 = ihasher(address.building);
...
struct PersonHasher {
    ...
    size_t operator()(const Person &person) const {
        size_t coef = 7340033;
        size_t r1 = shasher(person.name);
        size_t r2 = ihasher(person.height);
        size_t r3 = dhasher(person.weight);
        size_t r4 = ahasher(person.address);
        return coef * coef * coef * r1 ^ coef * coef * r2 ^ coef * r3 + r4;
    }
};
